...
int main(void)
{
    ...
    for (int r=0; r<5; r++)
    {
        ...
        for(int i = 0; i<quantity[r]-1; i++){if((array[i]-array[i+1])>-1e-12){tmpcount++;}} // Проверка сортировки
    ...
    for (int r=0; r<5; r++)
    {
        ...
        for(int i = 0; i<quantity[r]-1; i++){if((array[i]-array[i+1])>-1e-6){tmpcount++;}} // Проверка сортировки
