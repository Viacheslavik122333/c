...
double piecewise_quadratic_interpolation(double x, double xv[], double yv[], int n) 
{
    for (int i = 0; i < n - 2; i++) 
    {
        if (x >= xv[i] && x <= xv[i + 2]) 
        {
            ...
            double a = ((y1 - y0) * (x2 - x0) - (y2 - y0) * (x1 - x0)) / ((x1 - x0) * (x2 - x0) * (x2 - x1));
            double b = (y1 - y0 - a * (x1 * x1 - x0 * x0)) / (x1 - x0);
            double c = y0 - a * x0 * x0 - b * x0;

            // Вычисление значения квадратичной функции в точке x
            return a * x * x + b * x + c;
        }
    }
...
int test_linear_function(FILE* fout) 
{
    ...
    double yp = piecewise_quadratic_interpolation(xp, xv, yv, n);
...
int test_multiple_points(FILE* fout) 
{
    ...
    for (int i = 0; i < 3; i++) {
        double yp = piecewise_quadratic_interpolation(xp[i], xv, yv, n);
        printf( "Нужное значение: %lf Найденное значение: %lf\n", expected_yp[i], yp);
        fprintf(fout, "%.2lf %.2lf\n", xp[i], yp);
    }
