...
public:
    ...
    void print (int r=10) const 
    {
		print_tree(root, r, 0, 0);
	}

    io_status add_node(avl_tree_node<T> *newnode)
    {
        if(newnode == nullptr) return io_status::memory;
        int flag = 0;
        io_status tmp = io_status::success;
        root = insert(root, newnode, flag, tmp);

        if(tmp != io_status::success) return tmp;
    ...
    {
        avl_tree_node<T>* node = _search(root, key);
        return node ? node->get_name() : nullptr;
    }

private:
    avl_tree_node<T>* _search(avl_tree_node<T>* node, const char* key) 
    {
        if (!node) return nullptr;
        int cmp = strcmp(key, node->get_value());
        if (cmp < 0) return _search(node->left, key);
        if (cmp > 0) return _search(node->right, key);
        return node;
    }
    ...
    avl_tree_node<T>* insert (avl_tree_node<T> *root, avl_tree_node<T> *x, int &flag, io_status &tmp)
    {
        if(root == nullptr) 
        {
            flag = 1;
        ...
        if(*x < *root)
        {
            root->left = insert(root->left, x, flag, tmp);
            if(flag == 0)
                return root;
            switch(root->balance) 
            {
                case 1:
                    root->balance = 0;  flag = 0;
                    return root;
                case 0:
                    root->balance = -1;  flag = 1;
                    return root;
                case -1:
                    return rotate_right(root, flag);
                default:
                    return nullptr;
            }
        }
        ...
        {
            root->right = insert(root->right, x, flag, tmp);
            if(flag == 0)
                return root;
            switch(root->balance) 
            {
